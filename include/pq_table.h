...
//_rearrangement_perm.bin
...
#ifdef EXEC_ENV_OLS
        diskann::load_bin<uint32_t>(files,
                                    pq_pivots_path + "_chunk_offsets.bin",
                                    chunk_offsets, nr, nc);
#else
    ...
    void load_pq_pivots_new(const std::string &pq_pivots_path,
        ...
        diskann::load_bin<_u64>(pq_pivots_path, file_offset_data, nr, nc,
                                offset);
#endif
...
#ifdef EXEC_ENV_OLS
        diskann::load_bin<float>(files, pq_pivots_path.c_str(), centroid, nr,
                                 nc, file_offset_data[1] + offset);
#else
