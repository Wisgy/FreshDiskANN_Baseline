        ...
        while (n_tries <= n_retries) {
            ...
            if (ret != (int64_t)n_ops) {
                std::cerr << "io_submit() failed; returned " << ret
                          ...
                          << "=" << ::strerror((int)-ret) << ", try #"
                          << n_tries + 1;
                diskann::cout << "ctx: " << ctx << "\n";
                exit(-1);
            } else {
                // wait on io_getevents
                ret = io_getevents(ctx, (int64_t)n_ops, (int64_t)n_ops,
                                   evts.data(), nullptr);
...
LinuxAlignedFileReader::~LinuxAlignedFileReader() {
    ...
    if (ret == -1) {
        if (errno != EBADF) {
            ...
            // error checks
            if (ret == -1) {
                std::cerr << "close() failed; returned " << ret
...
void LinuxAlignedFileReader::register_thread() {
    ...
    if (ret != 0) {
        ...
        assert(errno != EAGAIN);
        assert(errno != ENOMEM);
        std::cerr << "io_setup() failed; returned " << ret
                  << ", errno=" << errno << ":" << ::strerror(errno)
                  << std::endl;
    } else {
        ctx_map[my_id] = ctx;
