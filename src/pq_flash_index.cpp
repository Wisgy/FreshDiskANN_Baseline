...
PQFlashIndex<T, TagT>::PQFlashIndex(
    ...
    if (m == diskann::Metric::COSINE) {
        if (std::is_floating_point<T>::value) {
            ...
            diskann::cerr
                << "WARNING: Cannot normalize integral data types."
...
#pragma omp critical
        {
            ...
            _u64 coord_alloc_size =
                ROUND_UP(MAX_N_CMPS * this->aligned_dim, 256);
    ...
    while ((node_list.size() + cur_level->size() < num_nodes_to_cache) &&
        ...
        for (size_t block = 0; block < nblocks && !finish_flag; block++) {
            ...
            for (size_t cur_pt = start; cur_pt < end; cur_pt++) {
                ...
                read.buf = buf;
                read.offset =
                    NODE_SECTOR_NO(nodes_to_expand[cur_pt]) * SECTOR_LEN;
                read_reqs.push_back(read);
            }
            ...
            for (auto &nhood : nhoods) {
                ...
                for (_u64 j = 0; j < nnbrs && !finish_flag; j++) {
                    if (std::find(node_list.begin(), node_list.end(),
                                  nbrs[j]) == node_list.end()) {
                        cur_level->insert(nbrs[j]);
                    }
                    if (cur_level->size() + node_list.size() >=
                        num_nodes_to_cache) {
                        finish_flag = true;
                    }
                }
...
int PQFlashIndex<T, TagT>::load(const char *index_prefix, _u32 num_threads,
    ...
    if (false == this->single_index_file) {
        ...
        // or centroids file (or medoids/centroid data stored in
        // disk_index_file).
...
// read index metadata
    ...
    if (file_exists(medoids_file)) {
        size_t tmp_dim;
        diskann::load_bin<uint32_t>(medoids_file, medoids, num_medoids,
                                    tmp_dim);
#endif
        ...
        if (tmp_dim != 1) {
            ...
            stream
                << "Error loading medoids file. Expected bin format of m times "
        ...
        if (!file_exists(centroids_file)) {
...
#ifdef EXEC_ENV_OLS
            diskann::load_aligned_bin<float>(files, centroids_file,
                                             centroid_data, num_centroids,
            ...
            diskann::load_aligned_bin<float>(centroids_file, centroid_data,
                                             ...
                                             aligned_tmp_dim);
#endif
            if (aligned_tmp_dim != aligned_dim ||
                num_centroids != num_medoids) {
                std::stringstream stream;
                stream
                    << "Error loading centroids data file. Expected bin format "
                       "of "
    ...
    while (k < cur_list_size && num_ios < IO_LIMIT) {
        ...
        for (auto &cached_nhood : cached_nhoods) {
            ...
            if (!exclude_cur_node) {
                ...
                if (coord_map != nullptr) {
                    coord_map->insert(std::make_pair(cached_nhood.first,
                                                     node_fp_coords_copy));
                }
            ...
            for (_u64 m = 0; m < nnbrs; ++m) {
                ...
                if (visited.find(id) != visited.end()) {
                    ...
                    auto r = InsertIntoPool(retset.data(), cur_list_size,
                                            nn); // Return position in sorted
                                                 // list where nn inserted.
                    if (cur_list_size < l_search)
                        ++cur_list_size;
                    if (r < nk)
                        nk = r; // nk logs the best position in the retset that
                                // was
                                // updated
        ...
        for (auto &frontier_nhood : frontier_nhoods) {
            ...
            if (!exclude_cur_node) {
                // added for StreamingMerger calls
                if (coord_map != nullptr) {
                    coord_map->insert(std::make_pair(frontier_nhood.first,
                                                     node_fp_coords_copy));
                }
            ...
            for (_u64 m = 0; m < nnbrs; ++m) {
                ...
                if (visited.find(id) != visited.end()) {
                    ...
                    if (r < nk)
                        nk = r; // nk logs the best position in the retset that
                                // was updated due to neighbors of n.
                }
            }
...
_u32 PQFlashIndex<T, TagT>::merge_read(std::vector<DiskNode<T>> &disk_nodes,
    ...
    for (_u32 i = 0; i < n_sectors; i++) {
        ...
        for (_u32 j = 0;
             j < nnodes_per_sector && cur_node_id < this->num_points; j++) {
        ...
        for (uint32_t i = 0;
            ...
            for (uint32_t j = 0;
                ...
                if (delete_set.find(n_scanned) != delete_set.end()) {
                    ...
                    DiskNode<T> node(n_scanned,
                                     OFFSET_TO_NODE_COORDS(buf_start),
                                     OFFSET_TO_NODE_NHOOD(buf_start));
                    /*            if ((!(node.nnbrs > 0)) ||
                                    ((n_scanned >= 325000) && (n_scanned <
                       325100)) ||
                                    ((n_scanned >= 300000) && (n_scanned <
                       300100))) { std::cout << "#neighbors of " << n_scanned <<
                       "   :  "
                                            << node.nnbrs << std::endl;
                                  std::cout << NODE_SECTOR_NO(n_scanned) <<
                       std::endl; std::cout << uint32_t(node_buf - sector_buf)
                       << std::endl; for (size_t i = 0; i < 128; i++) {
                       std::cout <<
                       *(OFFSET_TO_NODE_COORDS(buf_start) + i)
                                              << "  ;    ";
                                  }
                                  std::cout << std::endl;
                                  if (((n_scanned % 5) != 0) && (node.nnbrs ==
                       0)) { std::cout << "Previous vector : " << std::endl;
                                    std::cout << NODE_SECTOR_NO(n_scanned - 1)
                       << std::endl; char *buf_prev = OFFSET_TO_NODE(sector_buf,
                       n_scanned
                       - 1); std::cout << uint32_t(buf_prev - sector_buf) <<
                       std::endl; for (size_t i = 0; i < 128; i++) { std::cout
                       << *(OFFSET_TO_NODE_COORDS(buf_prev) + i)
                                                << "  ;    ";
                                    }
                                    std::cout << std::endl;
                                  }
...
int PQFlashIndex<T, TagT>::get_vector_by_tag(const TagT &tag,
                                             T *vector_coords) {
    if (!enable_tags) {
        diskann::cout << "Tags are disabled, cannot retrieve vector"
                      << std::endl;
        return -1;
    }
    size_t pos;
