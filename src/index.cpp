...
_u64 Index<T, TagT>::save_tags(std::string tags_file, size_t offset) {
    if (!_enable_tags) {
        diskann::cout << "Not saving tags as they are not enabled."
                      << std::endl;
        return 0;
    }
...
void Index<T, TagT>::save(const char *filename) {
    ...
    if (!_save_as_one_file) {
        ...
        diskann::save_bin<_u64>(filename, cumul_bytes.data(),
                                cumul_bytes.size(), 1, 0);

        diskann::cout << "Saved index as one file to " << filename
            ...
            if (_dynamic_index) {
                ...
                for (unsigned m = 0; m < _final_graph[n].size(); m++) {
                    if (_final_graph[n][m] >= _max_points + _num_frozen_pts) {
                        diskann::cerr
                            << "Wrong id found: " << _final_graph[n][m]
                            << std::endl;
                        throw diskann::ANNException(
...
void Index<T, TagT>::prune_neighbors(const unsigned location,
    ...
    if (_saturate_graph && alpha > 1) {
        for (uint32_t i = 0; i < pool.size() && pruned_list.size() < range;
             i++) {
            if ((std::find(pruned_list.begin(), pruned_list.end(),
                           pool[i].id) == pruned_list.end()) &&
                pool[i].id != location)
...
void Index<T, TagT>::batch_inter_insert(
    ...
    for (auto des : pruned_list) {
        ...
        if (des > _max_points)
            diskann::cout << "error. " << des << " exceeds max_pts"
                          << std::endl;
        /* des_pool contains the neighbors of the neighbors of n */

        {
            LockGuard guard(_locks[des]);
            if (std::find(_final_graph[des].begin(), _final_graph[des].end(),
                          n) == _final_graph[des].end()) {
...
void Index<T, TagT>::inter_insert(unsigned n,
    ...
    for (auto des : src_pool) {
        ...
        {
            LockGuard guard(_locks[des]);
            if (std::find(des_pool.begin(), des_pool.end(), n) ==
                des_pool.end()) {
        ...
        if (prune_needed) {
            ...
            size_t reserveSize =
                (size_t)(std::ceil(1.05 * SLACK_FACTOR * range));
                    ...
                    for (auto out_nbr : _final_graph[des]) {
                        {
                            ...
                            for (unsigned i = 0; i < _in_graph[out_nbr].size();
                                 i++) {
                                if (_in_graph[out_nbr][i] == des) {
                                    _in_graph[out_nbr].erase(
                                        _in_graph[out_nbr].begin() + i);
                                    break;
                                }
                            }
                        }
...
#pragma omp parallel for schedule(dynamic, 65536)
                    ...
                    for (auto cur_nbr : _final_graph[node]) {
                        if (dummy_visited.find(cur_nbr) ==
                            ...
                            float dist = _distance->compare(
                                ...
                                _data + _aligned_dim * (size_t)cur_nbr,
                                (unsigned)_aligned_dim);
                            dummy_pool.emplace_back(
                                Neighbor(cur_nbr, dist, true));
                            dummy_visited.insert(cur_nbr);
                        }
                    }
                    prune_neighbors(node, dummy_pool, parameters,
                                    new_out_neighbors);
            ...
            if ((sync_num * 100) / NUM_SYNCS > progress_counter) {
                ...
                diskann::cout
                    << "Completed  (round: " << rnd_no << ", sync: " << sync_num
                    << "/" << NUM_SYNCS << " with L " << L << ")"
...
#endif
        if (_nd > 0) {
            diskann::cout << "Completed Pass " << rnd_no
                          << " of data using L=" << L
    ...
    if (filename == nullptr) {
        ...
        if (file_num_points > _max_points ||
            ...
            stream << "ERROR: Driver requests loading " << num_points_to_load
                   << " points and file has " << file_num_points
                   << " points, but "
            ...
            if (tag_filename == nullptr) {
                ...
                if (file_exists(tag_filename)) {
                    ...
                    if (npts != num_points_to_load) {
                        ...
                        throw diskann::ANNException(
                            sstream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
                    }
                    for (size_t i = 0; i < npts; i++) {
                    ...
                    diskann::cerr << "Tag file " << tag_filename
                                  << " does not exist. Exiting..." << std::endl;
                    throw diskann::ANNException(
                        std::string("Tag file") + tag_filename +
                            " does not exist",
                        -1, __FUNCSIG__, __FILE__, __LINE__);
                }
            }
...
int Index<T, TagT>::eager_delete(const TagT tag, const Parameters &parameters,
    ...
    if (_lazy_done && (!_data_compacted)) {
        diskann::cout
            << "Lazy delete requests issued but data not consolidated, "
    ...
    for (size_t i = 0; i < intersection.size(); i++) {
...

        {
            ...
            // constructing candidate set from out-neighbors and out-neighbors
            // of ngh and id
            { // should a shared reader lock on delete_lock be held here at the
                // beginning of the two for loops or should it be held and
                // release for ech iteration of the for loops? Which is faster?
                ...
                for (size_t i = 0; i < result.size(); i++) {
                    ...
                    if (_delete_set.find(j.id) == _delete_set.end()) {
                        _final_graph[ngh].push_back(j.id);
                        {
                            ...
                            if (std::find(_in_graph[j.id].begin(),
                                          _in_graph[j.id].end(),
        ...
        if (_num_frozen_pts > 0) {
            ...
            if (!_final_graph[_max_points].empty()) {
                ...
                for (unsigned k = 0; k < _final_graph[_max_points].size(); k++)
                    _final_graph[_nd].emplace_back(
                        _final_graph[_max_points][k]);
                ...
                memcpy((void *)(_data + (size_t)_aligned_dim * _nd),
                       _data + (size_t)_aligned_dim * _max_points,
                       sizeof(T) * _dim);
    ...
    for (unsigned old = 0; old <= _max_points; ++old) {
        if ((new_location[old] < _max_points) ||
            ...
            for (size_t i = 0; i < _final_graph[old].size(); ++i) {
                if (new_location[_final_graph[old][i]] > _final_graph[old][i]) {
                    ...
                    sstream << "Error in compact_data(). Found point: " << old
                            ...
                            << _final_graph[old][i];
                    if (_delete_set.find(_final_graph[old][i]) !=
                        _delete_set.end()) {
                        sstream << std::endl
                                << " Point: " << old << " index: " << i
                                << " neighbor: " << _final_graph[old][i]
                                << " found in delete set of size: "
            ...
            if (new_location[old] != old) {
                ...
                memcpy(
                    (void *)(_data + _aligned_dim * (size_t)new_location[old]),
...
int Index<T, TagT>::lazy_delete(const tsl::robin_set<TagT> &tags,
    ...
    if (failed_tags.size() > 0) {
        std::cerr << "failed_tags should be passed as an empty list"
                  << std::endl;
        return -3;
    }
    if ((_eager_done) && (!_data_compacted)) {
