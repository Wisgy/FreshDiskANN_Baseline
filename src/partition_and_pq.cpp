...
#pragma omp parallel for schedule(static, 65536)
        ...
        kmeans::run_lloyds(cur_data.get(), num_train, cur_chunk_size,
                           cur_pivot_data.get(), num_centers, k_means_reps,
    ...
    cumul_bytes[3] =
        cumul_bytes[2] + diskann::save_bin<uint32_t>(
                             pq_pivots_path.c_str(), rearrangement.data(),
                             rearrangement.size(), 1, cumul_bytes[2]);
    cumul_bytes[4] =
        cumul_bytes[3] + diskann::save_bin<uint32_t>(
...
int generate_pq_data_from_pivots(const std::string data_file,
    ...
    if (!file_exists(pq_pivots_path)) {
        ...
        diskann::load_bin<_u64>(pq_pivots_path.c_str(), file_offset_data, nr,
                                nc, 0);

        if (nr != 5) {
            diskann::cout
                << "Error reading pq_pivots file " << pq_pivots_path
                << ". Offsets dont contain correct metadata, # offsets = " << nr
                << ", but expecting 5.";
            throw diskann::ANNException(
                "Error reading pq_pivots file at offsets data.", -1,
                __FUNCSIG__, __FILE__, __LINE__);
        }

        diskann::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr,
                                 nc, file_offset_data[0]);

        if ((nr != num_centers) || (nc != dim)) {
            diskann::cout << "Error reading pq_pivots file " << pq_pivots_path
                          << ". file_num_centers  = " << nr
                          << ", file_dim = " << nc << " but expecting "
                          << num_centers << " centers in " << dim
                          << " dimensions.";
            throw diskann::ANNException(
        ...
        if ((nr != dim) || (nc != 1)) {
            diskann::cout << "Error reading pq_pivots file " << pq_pivots_path
                          ...
                          << " but expecting " << dim
                          << " entries in 1 dimension.";
            throw diskann::ANNException(
                "Error reading pq_pivots file at centroid data.", -1,
                __FUNCSIG__, __FILE__, __LINE__);
        }

        diskann::load_bin<uint32_t>(pq_pivots_path.c_str(), rearrangement, nr,
        ...
        diskann::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr,
                                    nc, file_offset_data[3]);
        ...
        if (nr != (uint64_t)num_pq_chunks + 1 || nc != 1) {
            diskann::cout
                ...
                << ", nc=1." << std::endl;
            throw diskann::ANNException(
                "Error reading pq_pivots file at chunk offsets.", -1,
                __FUNCSIG__, __FILE__, __LINE__);
        }
    ...
    for (size_t block = 0; block < num_blocks; block++) {
        ...
        diskann::convert_types<T, float>(
            block_data_T.get(), block_data_tmp.get(), cur_blk_size, dim);

        /*    diskann::cout << "Processing points  [" << start_id << ", " <<
           end_id
                          << ")... Used memory: "
...
#pragma omp parallel for schedule(static, 8192)
        ...
        if (num_centers > 256) {
            ...
            diskann::convert_types<uint32_t, uint8_t>(
                block_compressed_base.get(), pVec.get(), cur_blk_size,
                num_pq_chunks);
            compressed_file_writer.write((char *)(pVec.get()),
                                         cur_blk_size * num_pq_chunks *
                                             sizeof(uint8_t));
        }
...
int estimate_cluster_sizes(const std::string data_file, float *pivots,
    ...
    if (test_dim != dim) {
        diskann::cout
            << "Error. dimensions dont match for pivot set and base set"
            << std::endl;
        return -1;
    }
...
int shard_data_into_clusters(const std::string data_file, float *pivots,
    ...
    if (basedim32 != dim) {
        diskann::cout
            << "Error. dimensions dont match for train set and base set"
            << std::endl;
        return -1;
    }
    ...
    for (size_t block = 0; block < num_blocks; block++) {
        ...
        math_utils::compute_closest_centers(
            block_data_float.get(), cur_blk_size, dim, pivots, num_centers,
            k_base, block_closest_centers.get());
...
int partition_with_ram_budget(const std::string data_file,
    ...
    while (!fit_in_ram) {
        ...
        diskann::cout << "Processing global k-means (kmeans_partitioning Step)"
                      << std::endl;
        kmeans::kmeanspp_selecting_pivots(train_data_float, num_train,
                                          train_dim, pivot_data, num_parts);
