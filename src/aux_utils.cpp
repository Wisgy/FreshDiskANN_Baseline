...
double get_memory_budget(double search_ram_budget) {
    ...
    if (search_ram_budget - SPACE_FOR_CACHED_NODES_IN_GB >
        ...
        final_index_ram_limit =
            search_ram_budget - SPACE_FOR_CACHED_NODES_IN_GB;
    }
...
T *load_warmup(MemoryMappedFiles &files, const std::string &cache_warmup_file,
    ...
    if (files.fileExists(cache_warmup_file)) {
        diskann::load_aligned_bin<T>(files, cache_warmup_file, warmup,
                                     warmup_num, file_dim, file_aligned_dim);
...
int build_merged_vamana_index(
    ...
    for (int p = 0; p < num_parts; p++) {
        ...
        std::string shard_index_file = merged_index_prefix + "_subshard-" +
                                       std::to_string(p) + "_mem.index";
...
uint32_t
optimize_beamwidth(std::unique_ptr<diskann::PQFlashIndex<T, TagT>> &pFlashIndex,
...
void create_disk_layout(const std::string &mem_index_file,
    ...
    if (single_file_index) {
        ...
        if (nr != Index<T, TagT>::METADATA_ROWS && nc != 1) {
            std::stringstream stream;
            stream << "Vamana Single Index file size does not meet meta-data "
                      "criteria."
                   << std::endl;
    ...
    unsigned *nhood_buf = (unsigned *)(node_buf.get() + (ndims_64 * sizeof(T)) +
                                       sizeof(unsigned));
    ...
    for (_u64 sector = 0; sector < n_sectors; sector++) {
        ...
        for (_u64 sector_node_id = 0;
            ...
            if (nnbrs == 0) {
                diskann::cout
                    << "ERROR. Found point with no out-neighbors; Point#: "
            ...
            base_reader.read((char *)cur_node_coords.get(),
                             sizeof(T) * ndims_64);
        ...
        if (tags_enabled) {
            ...
            if (single_file_index) {
                ...
                if (!file_exists(tag_file)) {
                    diskann::cout << "Static vamana index, tag file "
                                  << tag_file << "does not exist. Exiting...."
                                  << std::endl;
                    exit(-1);
                }
...
bool build_disk_index(const char *dataPath, const char *indexFilePath,
    ...
    if (final_index_ram_limit <= 0) {
        diskann::cerr
            << "Insufficient memory budget (or string was not in right "
               "format). Should be > 0."
            << std::endl;
        return false;
    }
