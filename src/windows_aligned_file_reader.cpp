...
void WindowsAlignedFileReader::read(std::vector<AlignedRead> &read_reqs,
    ...
    for (_u64 i = 0; i < n_batches; i++) {
        ...
        while (n_complete < batch_size) {
            if (GetQueuedCompletionStatus(ctx.iocp, &n_read, &completion_key,
                                          &lp_os, INFINITE) != 0) {
                ...
                if (lp_os == NULL) {
                    ...
                    if (error != WAIT_TIMEOUT) {
                        ...
                        throw diskann::ANNException(
                            "GetQueuedCompletionStatus failed with error: ",
                            error, __FUNCSIG__, __FILE__, __LINE__);
                    }
                    ...
                    std::stringstream stream;
                    stream << "I/O failed , offset: "
                           << read_reqs[op_idx].offset
