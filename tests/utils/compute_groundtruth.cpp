...
#pragma omp parallel for schedule(static, 65536)
    for (int64_t d = 0; d < num_points; ++d)
        points_l2sq[d] =
            cblas_sdot(dim, matrix + (ptrdiff_t)d * (ptrdiff_t)dim, 1,
               ...
               const float *const queries) // queries in Col major
{
    ...
    for (_u64 b = 0; b < div_round_up(nqueries, q_batch_size); ++b) {
        ...
        distsq_to_points(dim, dist_matrix, npoints, points, points_l2sq,
                         ...
                         queries_l2sq + q_b);
        diskann::cout << "Computed distances for queries: [" << q_b << ","
                      << q_e << ")" << std::endl;
...
#pragma omp parallel for schedule(dynamic, 16)
            ...
            for (_u64 p = k; p < npoints; p++) {
                if (point_dist.top().second >
                    dist_matrix[(ptrdiff_t)p +
                                (ptrdiff_t)(q - q_b) * (ptrdiff_t)npoints])
                    point_dist.emplace(
                        p, dist_matrix[(ptrdiff_t)p + (ptrdiff_t)(q - q_b) *
                                                          (ptrdiff_t)npoints]);
        ...
        diskann::cout << "Computed exact k-NN for queries: [" << q_b << ","
                      << q_e << ")" << std::endl;
    }
...
template <typename T> int aux_main(int argc, char **argv) {
    ...
    for (int p = 0; p < num_parts; p++) {
        ...
        exact_knn(dim, k, closest_points_part, dist_closest_points_part,
