...
void run(const uint32_t base_count, const uint32_t num_mem_indices,
    ...
    while (count++ < num_cycles) {
        ...
        for (auto iter = active_tags.begin(); iter != active_tags.end();
             iter++) {
        ...
        for (uint64_t i = 0; i < num_mem_indices; i++) {
            ...
            dump_to_disk(all_points, ndims,
                         out_prefix + "_cycle_" + std::to_string(count) +
                             "_mem_" + std::to_string(i + 1),
                         mem_tags[i]);
        }
        ...
        //  std::vector<uint64_t> rem_tags(tags.begin() + delete_count,
        //  tags.end()); std::sort(rem_tags.begin(), rem_tags.end());
        // write mem
