...
void extract_rows(const std::string &input_file, _u64 nrows, _u64 ndims,
    ...
    if (replace_content) {
        std::cout << std::endl
                  << "Replacing file content with random values...";
        ...
        std::cout << nrows_to_extract * ndims << " values replaced. "
                  << std::endl;
    } else {
    ...
    if (has_id) {
        ...
        _u64 tag_start = 2 * sizeof(_u32) + nrows * ndims * sizeof(T) +
                         offset * sizeof(_u64);
        fin.seekg(tag_start);
...
int main(int argc, char *argv[]) {
    if (argc != 6 && argc != 7 && argc != 8) {
        std::cout
            << "Usage: <program> <input_file> <datatype> (uint8|int8|float) "
               ...
               "(zero if beginning) <output_file> [has_tags(true/false) "
               "default is "
               "false, file doesn't have ids.] [replace_random (true/false) "
               "default is false, i.e.don't replace file content with random "
               "data]"
            << std::endl;
    }
    ...
    if (argc >= 8) {
        if (std::string(argv[count++]) == "true") {
            if (!has_id || datatype != "float") {
                std::cout << "Can replace content ONLY if has_id is true and "
                             "datatype "
                             "== float. "
                          << std::endl;
                return -6;
    ...
    if (datatype == "uint8") {
        extract_rows<uint8_t>(input_file, nrows, ncols, offset,
                              nrows_to_extract, output_file, has_id,
                              replace_content);
