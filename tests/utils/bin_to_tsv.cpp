...
int main(int argc, char **argv) {
    ...
    if ((type_string != std::string("float")) &&
        ...
        std::cerr << "Error: type not supported. Use float/int8/uint8"
                  << std::endl;
    }
    ...
    for (_u64 i = 0; i < nblks; i++) {
        ...
        if (type_string == std::string("float"))
            block_convert<float>(writer, reader, (float *)read_buf, cblk_size,
                                 ndims);
            ...
            block_convert<uint8_t>(writer, reader, (uint8_t *)read_buf,
                                   cblk_size, ndims);
