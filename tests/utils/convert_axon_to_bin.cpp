...
uint32_t numPointsToCheck(uint32_t numPoints) {
    ...
    return point01percent > MIN_POINTS_FOR_VALUE_VERIFICATION
               ...
               : point1percent > MIN_POINTS_FOR_VALUE_VERIFICATION
                     ? point1percent
                     : onepercent > MIN_POINTS_FOR_VALUE_VERIFICATION
                           ? onepercent
                           : numPoints;
}
...
void sanityCheck(const char *origfile, const char *binfile,
    ...
    if (nd != numDimensions) {
        throw diskann::ANNException(
            "Verifying binary write failed! nd != numDimensions", -1,
    ...
    for (uint32_t i = 0; i < vectorsToCheck; i++) {
        ...
        for (uint32_t j = 0; j < nd; j++) {
            if (origvec[j] != data[i * nd + j]) {
                std::cerr << "Found mismatch at line " << i
                          << " orig: " << origvec[j]
    ...
    if (mismatchCount == 0) {
        diskann::cout << "Successfully verified byte-byte conversion."
                      << std::endl;
    } else {
...
int main(int argc, char **argv) {
    ...
    diskann::cout << "Converting (" << argv[1] << ") axon file: " << argv[2]
                  << " with " << argv[4]
                  << " points to diskann bin format file " << argv[3]
                  << std::endl;
