...
void concat_files(int argc, char **argv) {
    ...
    if (dimension1 != dimension2) {
        diskann::cout
            << "Error! Cannot combine vectors of differing dimensions ("
...
void compareDistanceComputationsInt() {
...

    {
        ...
        if (abs(dist1 - dist2) > epsilon) {
            std::cout << "compareDistanceComputationsInt(): L2 Test failed. "
                         "AVX dist: "
    ...
    }
    {
        ...
        if (abs(dist1 - dist2) > epsilon) {
            std::cout << "compareDistanceComputationsInt(): Cosine Test "
                         "failed. AVX dist: "
                      << dist1 << " normal dist: " << dist2 << " difference > "
                      << epsilon << std::endl;
        } else {
...
void compareDistanceComputationsFloat() {
...

    {
        ...
        if (abs(dist1 - dist2) > epsilon) {
            std::cout << "compareDistanceComputationsFloat(): L2 Test failed. "
                         "AVX dist: "
                      << dist1 << " normal dist: " << dist2 << " difference > "
                      << epsilon << std::endl;
        } else {
    ...
    }
    {
        ...
        if (abs(dist1 - dist2) > epsilon) {
            std::cout
                << "compareDistanceComputationsFloat(): Cosine Test failed. "
                   "AVX dist: "
                << dist1 << " normal dist: " << dist2 << " difference > "
                << epsilon << std::endl;
        } else {
...
int main(int argc, char **argv) {
    ...
    case 7:
        ...
        break;
    default:
        diskann::cout << "Don't know what to do with mode parameter: "
                      << argv[1] << std::endl;
    }
}
