...
#pragma omp parallel for
        for (_s64 p = 0; p < (_s64)reinsert_vec.size(); p++) {
            index.insert_point(data_load + (size_t)(reinsert_vec[p]) *
                                               (size_t)aligned_dim,
...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == std::string("int8"))
        build_incremental_index<int8_t>(argv[2], L, R, alpha, save_path,
                                        num_incr);
    else if (std::string(argv[1]) == std::string("uint8"))
        build_incremental_index<uint8_t>(argv[2], L, R, alpha, save_path,
                                         num_incr);
    else if (std::string(argv[1]) == std::string("float"))
        build_incremental_index<float>(argv[2], L, R, alpha, save_path,
                                       num_incr);
    else
