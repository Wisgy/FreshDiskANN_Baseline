...
#pragma omp parallel for
        for (int64_t i = 0; i < (int64_t)query_num; i++) {
            ...
            index.search_with_tags(query + i * query_aligned_dim, recall_at,
    ...
    while (i < num_cycles) {
...

        {
            ...
            if (failed_tags.size() > 0) {
                std::cerr << "Failed to delete " << failed_tags.size()
                          << " tags" << std::endl;
            }
            /*
            ...
            search_kernel(query, query_num, query_aligned_dim, recall_at, Lvec,
                          index, truthset_file, active_tags);
        }
...
#pragma omp parallel for
            for (size_t i = 0; i < delete_vector.size(); i++) {
                ...
                index.insert_point(data_load + (size_t)p * (size_t)aligned_dim,
                                   paras, p);
            }
