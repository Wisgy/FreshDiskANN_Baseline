...
template <typename T> using IncrIndex = diskann::DiskANNIncrementalIndex<T>;
...
void runIncrementalIntfTest(const std::string &basefile,
  ...
  if (npts / maxMemIndexPoints < 7) {
      std::cerr << "Very few points to test the functionality thoroughly. "
                << "Either reduce the max points per index or select a bigger "
                   "base file"
                << std::endl;
      exit(1);
  }
...
int main(int argc, char **argv) {
    ...
    if (argc != 4) {
        std::cout
            << "Usage: <program> <datatype [float|int8|uint8]> <base_file> "
    ...
    if (datatype == "float") {
        ...
        runIncrementalIntfTest<uint8_t>(basefile, workingDir,
                                        maxMemIndexPoints);
