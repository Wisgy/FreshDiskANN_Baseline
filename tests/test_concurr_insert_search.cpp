...
int build_incremental_index(const std::string &data_path, const unsigned L,
                                 ...
                                 query_aligned_dim);
    {
        std::future<void> future =
            std::async(std::launch::async, insert_kernel<T, TagT>, data_load,
                       std::ref(index), std::ref(paras), num_points,
                       aligned_dim, num_prebuilt);
        ...
        do {
            ...
            if (status == std::future_status::deferred) {
                ...
                search_kernel(query, query_num, query_aligned_dim, recall_at,
                              Lvec, index);
                total_queries += query_num;
                diskann::cout << "total queries: " << total_queries
                              << std::endl;
...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == std::string("int8"))
        ...
        build_incremental_index<uint8_t>(argv[2], L, R, C, num_rnds, alpha,
                                         save_path, query_file, recall_at,
                                         Lvec);
