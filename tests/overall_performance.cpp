...
void sync_search_kernel(T *query, size_t query_num, size_t query_aligned_dim,
    ...
    for (_u32 q = 0; q < query_num; q++) {
        for (_u32 r = 0; r < (_u32)recall_at; r++) {
            query_result_tags[q * recall_at + r] =
                std::numeric_limits<TagT>::max();
            query_result_dists[q * recall_at + r] =
...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS)
    ...
    if (calRecall) {
        if (merged) {
            ...
            diskann::save_bin<TagT>(cur_result_path, query_result_tags,
                                    query_num, recall_at);
        } else {
            std::string cur_result_path =
        ...
        // recall = diskann::calculate_recall(query_num, gt_ids, gt_dists,
        // gt_dim,
...
#pragma omp parallel for num_threads(NUM_INSERT_THREADS)
    for (_s64 i = 0; i < (_s64)insert_vec.size(); i++) {
        sync_index.insert(data_load + aligned_dim * insert_vec[i],
                          insert_vec[i]);
    }
...
void update(const std::string &data_path, const unsigned L_mem,
    ...
    diskann::MergeInsert<T, TagT> sync_index(
        paras, dim, save_path + "_mem", save_path, save_path + "_merge",
    ...
    for (int i = 0; i < batch; i++) {
        ...
        do {
            insert_status =
                insert_future.wait_for(std::chrono::milliseconds(1));
            if (insert_status == std::future_status::deferred) {
                ...
                sync_search_kernel(query, query_num, query_aligned_dim,
                                   recall_at, Lsearch, sync_index,
                                   currentFileName, inactive_tags, res, false,
                                   false);
        ...
        if (i == batch - 1) {
            ...
            merge_future =
                std::async(std::launch::async, merge_kernel<T, TagT>,
...
void build(const std::string &data_path, const unsigned L_mem,
    ...
    std::cout << "Sync Index build time: " << timer.elapsed() / 1000000
              << "s\n";
...
int main(int argc, char **argv) {
    ...
    if (!updateIndex || buildIndex) {
        if (std::string(argv[1]) == std::string("int8"))
            build<int8_t, unsigned>(argv[2], L_mem, R_mem, alpha_mem, L_disk,
                                    R_disk, alpha_disk, num_start, num_shards,
                                    num_pq_chunks, nodes_to_cache, save_path);
        else if (std::string(argv[1]) == std::string("uint8"))
            build<uint8_t, unsigned>(argv[2], L_mem, R_mem, alpha_mem, L_disk,
                                     R_disk, alpha_disk, num_start, num_shards,
                                     num_pq_chunks, nodes_to_cache, save_path);
        else if (std::string(argv[1]) == std::string("float"))
            build<float, unsigned>(argv[2], L_mem, R_mem, alpha_mem, L_disk,
                                   R_disk, alpha_disk, num_start, num_shards,
                                   num_pq_chunks, nodes_to_cache, save_path);
        else
    ...
    if (updateIndex) {
        if (std::string(argv[1]) == std::string("int8")) {
            ...
            update<int8_t, unsigned>(full_data_path, L_mem, R_mem, alpha_mem,
                                     L_disk, R_disk, alpha_disk, step,
                                     num_start, num_pq_chunks, nodes_to_cache,
                                     save_path, query_file, truthset, recall_at,
                                     Lsearch, beam_width, &dist_cmp);
        } else if (std::string(argv[1]) == std::string("uint8")) {
            diskann::DistanceL2UInt8 dist_cmp;
            update<uint8_t, unsigned>(
                full_data_path, L_mem, R_mem, alpha_mem, L_disk, R_disk,
                alpha_disk, step, num_start, num_pq_chunks, nodes_to_cache,
                save_path, query_file, truthset, recall_at, Lsearch, beam_width,
                &dist_cmp);
        } else if (std::string(argv[1]) == std::string("float")) {
            diskann::DistanceL2 dist_cmp;
            update<float, unsigned>(full_data_path, L_mem, R_mem, alpha_mem,
                                    L_disk, R_disk, alpha_disk, step, num_start,
                                    num_pq_chunks, nodes_to_cache, save_path,
