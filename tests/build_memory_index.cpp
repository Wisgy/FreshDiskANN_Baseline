...
int build_in_memory_index(const std::string &data_path,
    ...
    diskann::cout << "Building in-memory index with parameters: data_file: "
                  ...
                  << " #threads: " << num_threads << ", using distance metric: "
                  << (distMetric == diskann::Metric::COSINE ? "cosine "
                                                            : "l2 ");
...
int main(int argc, char **argv) {
    if (argc != 12) {
        diskann::cout << "Usage: " << argv[0]
                      << " <data_type(int8/uint8/float)>  <data_file.bin>"
                         ...
                         "<output_index_file> <dynamic_index(0/1)> "
    ...
    if (std::string(argv[1]) == std::string("int8"))
        build_in_memory_index<int8_t>(data_path, tags_file, R, L, alpha,
                                      save_path, num_threads, dynamic_index,
                                      single_file_index, distMetric);
    else if (std::string(argv[1]) == std::string("uint8"))
        build_in_memory_index<uint8_t>(data_path, tags_file, R, L, alpha,
                                       save_path, num_threads, dynamic_index,
                                       single_file_index, distMetric);
    else if (std::string(argv[1]) == std::string("float"))
        build_in_memory_index<float>(data_path, tags_file, R, L, alpha,
                                     save_path, num_threads, dynamic_index,
                                     single_file_index, distMetric);
    else
