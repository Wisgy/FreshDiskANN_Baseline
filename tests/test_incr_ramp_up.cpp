...
int build_incremental_index(const std::string &data_path, const unsigned L,
    ...
    diskann::Index<T, TagT> index(diskann::L2, dim, num_points, num_frozen,
                                  true, true);
    diskann::cout << "num_points = " << num_points;
    {
        ...
        diskann::cout << "Index build time: " << timer.elapsed() / 1000
                      << "ms\n";
    }
    ...
    for (unsigned i = 0; i < rounds; i++) {
        ...
        auto save_path_inc =
            save_path + ".inc" + std::to_string(i); // 10 -denotes that 10% of
                                                    // base points are being
                                                    // inserted at a time
        ...
        for (auto p : deleted_tags) {
            if (del_mode == 0) {
                if (index.eager_delete(p, paras) != 0) {
                    std::cerr << "Delete tag " << p << " not found"
                              << std::endl;
                } else
                    used_tags.erase(p);
            } else {
        ...
        diskann::cout << "Delete time in this phase "
                      << del_timer.elapsed() / 1000 << "ms\n";
        deleted_tags.clear();
        inserted_tags.clear();
...

    {
        ...
        diskann::cout << "Incremental time: " << timer.elapsed() / 1000
                      << "ms\n";
...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == std::string("int8"))
        ...
        build_incremental_index<float>(argv[2], L, R, C, num_rnds, alpha,
                                       save_path, num_incr, num_frozen,
                                       del_mode);
    else
