...
template <typename T> int search_disk_index(int argc, char **argv) {
    ...
    if (beamwidth <= 0)
        diskann::cout << "beamwidth to be optimized for each L value"
                      << std::endl;
    else
...
//  reader.reset(new diskann::MemAlignedFileReader());
    ...
    if (tags_flag) {
        ...
        diskann::cout << "Loaded " << active_tags.size()
                      << " tags from index for recall measurement."
                      << std::endl;
    } else {
        diskann::cout << "Not loading tags since they are disabled."
                      << std::endl;
    }
    ...
    for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++) {
        ...
        if (beamwidth <= 0) {
            ...
            optimized_beamwidth = optimize_beamwidth(
                _pFlashIndex, warmup, warmup_num, warmup_aligned_dim, (_u32)L,
                optimized_beamwidth);
        } else
...
#pragma omp parallel for schedule(dynamic, 1)
        for (_s64 i = 0; i < (int64_t)query_num; i++) {
            _pFlashIndex->cached_beam_search(
                query + (i * query_aligned_dim), (uint64_t)recall_at,
                (uint64_t)L, query_result_tags_32.data() + (i * recall_at),
        ...
        */
        ...
        diskann::cout << std::setw(6) << L << std::setw(12)
                      << optimized_beamwidth << std::setw(16) << qps
                      << std::setw(16) << mean_latency << std::setw(16)
                      << latency_999 << std::setw(16) << mean_ios
    ...
    for (auto L : Lvec) {
        ...
        diskann::save_bin<_u32>(cur_result_path,
                                query_result_ids[test_id].data(), query_num,
                                recall_at);

        cur_result_path =
            result_output_prefix + "_" + std::to_string(L) + "_tags_uint32.bin";
        diskann::save_bin<_u32>(cur_result_path,
                                query_result_tags[test_id].data(), query_num,
                                recall_at);
        cur_result_path =
    ...
    if (argc < 14) {
        diskann::cout
            ...
            << " <index_type (float/int8/uint8)>  <index_prefix_path>"
               ...
               " <L1>  [L2] etc.  See README for more information on "
               "parameters."
            << std::endl;
        exit(-1);
    }
