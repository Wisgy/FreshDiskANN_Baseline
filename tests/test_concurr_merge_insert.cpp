...
void seed_iter(tsl::robin_set<uint32_t> &active_set,
    ...
    std::vector<uint32_t> inactive_vec(inactive_set.begin(),
                                       inactive_set.end());
    ...
    std::ofstream inserted_points_writer(inserted_points_file,
                                         std::ios::binary);
...
void search_kernel(diskann::MergeInsert<T> &merge_insert,
    ...
    if (print_stats) {
        ...
        std::cout << std::setw(4) << "Ls" << std::setw(12) << "QPS "
                  ...
                  << "90 Latency" << std::setw(12) << "95 Latency"
...
void insertion_kernel(diskann::MergeInsert<T> &merge_insert,
    ...
    if (::_insertions_done.load()) {
        std::cout
            << "Insertions_done is true at the beginning of insertion kernel"
            << std::endl;
        exit(-1);
    }
    ...
    if (tag_num != npts) {
        std::cout
            << "In insertion_kernel(), number of tags loaded is not equal to "
...
template <typename T> void merge_kernel(diskann::MergeInsert<T> &merge_insert) {
...
void run_iter(diskann::MergeInsert<T> &merge_insert,
    ...
    if (::_insertions_done.load() && ::_del_done.load()) {
        ...
        seed_iter<T, TagT>(active_set, inactive_set, mem_pts_file,
                           mem_tags_file, deleted_tags);
        ::delete_future =
            std::async(std::launch::async, deletion_kernel<T, TagT>,
...
void run_all_iters(std::string base_prefix, std::string merge_prefix,
    ...
    for (size_t i = 0; i < tag_num; i++) {
        if (tag_data[i] != std::numeric_limits<uint32_t>::max()) {
            ...
            if (del_tags_found < 5)
                std::cout
                    << "Driver file found invalid tag in active tag file : "
                    << tag_data[i] << std::endl;
            del_tags_found++;
        }
    }
